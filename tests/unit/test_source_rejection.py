...
def test_dataset_cli_source_reject_records_reason_and_keeps_training_disabled(
    tmp_path: Path,
) -> None:
    ...
    assert main([
        "dataset", "source", "reject",
        "--pilot-dir", str(pilot_dir),
        "--source", str(source.source_asset_id),
        "--reason", "outside-launch-video-domain",
    ]) == 0
